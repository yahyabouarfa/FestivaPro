...
def upgrade() -> None:
    ...
    if not has_column("events", "attendance_count"):
        op.add_column("events", sa.Column("attendance_count", sa.Integer(), nullable=False, server_default="0"))

    created_event_salaries = not has_table("event_salaries")
    if created_event_salaries:
        op.create_table(
            "event_salaries",
    ...
    if not has_table("bartender_sales"):
        op.create_table(
            "bartender_sales",
            sa.Column("id", sa.Integer(), nullable=False),
            sa.Column("event_id", sa.Integer(), nullable=False),
    ...
    created_price_history = not has_table("price_history")
